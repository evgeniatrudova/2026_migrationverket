...
def get_comparative_criminology(state: str, current_year: int) -> dict:
    """Generates comparative data with a 30-year trend baseline."""
    
    # 30-year deterministic trend for Sweden (Historical decline, recent stabilization)
    swe_trend = []
    for i in range(30):
        if i < 15:
            val = 4.5 - (i * 0.1)
    ...
    # 30-year deterministic trend for Target State (Volatility & Escalation)
    state_trend = []
    base_historical = hate * 0.6
    for i in range(30):
        noise = ((state_hash + i) % 9 - 4) / 10.0
        if i < 20: # Historical volatility
            val = base_historical + (i * 0.05) + noise
        else: # Recent escalation
            val = base_historical + 1.0 + ((i - 20) * ((hate - (base_historical + 1.0)) / 9.0)) + noise
        
        if i == 29: 
            val = hate # Lock current year to exact generated metric
    ...
    if st.button(t["btn_run"], type="primary"):
        with st.spinner("Processing..."):
            ...
            fig_trend.update_layout(
                ...
                height=350, 
                yaxis_title="Incidentfrekvens (per 100k)",
                xaxis_title="År",
                margin=dict(t=40, b=0, l=0, r=0),
                hovermode="x unified", # Enables the vertical line comparing both values instantly
